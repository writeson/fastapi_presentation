...
async def read_albums(offset: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
    async with db as session:
...
async def read_album_with_tracks(id: int = Path(..., title="The ID of the album to get"), db: AsyncSession = Depends(get_db)):
    async with db as session:
